  ...
  die("Connection failed: " . $conn->connect_error);
		...
		if (is_array($fetchData)) {
		  ...
		  foreach ($fetchData as $data) {
			...
			echo '<td  style="text-align: center; border: 1.5px solid black;">' . $data['Humid'] .'%' .'</td>';
			echo '<td  style="text-align: center; border: 1.5px solid black;">' . $data['RTD'] . '&deg;C' .'</td>';
			echo '<td  style="text-align: center; border: 1.5px solid black;">' . $data['CO2'] . ' ppm'.'</td>';
